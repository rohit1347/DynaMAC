...
class QLearningEGreedy:
	...
	def __init__(self, prot, learn_rate = 0.3, discount = 0.8, epsilon = 0.7):
...
class decision_final:
	...
	# Aggregator
		...
		if mode == 2:
			...
			somac = QLearningBoltzmann(prot = portid, learn_rate = 0.7, discount = 0.8, T = 1)
...
#line4, = plt.plot(prob[1], label = 'Boltzmann Probability')
#line5, = plt.plot(prob)
plt.xlabel('Time index')
plt.ylabel('Value')	
